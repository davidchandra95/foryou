...
if(!empty($nama)) //jika ada file yang di attach
{
	...
	$lokasiakhir="assets/img/info/$namafile"; //untuk di database;
	$lokasifile="../../assets/img/info/$namafile"; //untuk di folder;
	...
	if (!in_array($tipe, $allowedExts))
	{
		...
		<?php echo"window.location='../home.php?p=info&addinfo'"; ?>
		</script>
		<?php
		exit();
	}
	...
	{ //jika filenya benar
		// $qry = mysql_query("select idinfo from info");
		...
		$qry = mysql_query("insert into info(title,des,url) values ('$judul','$isi','$lokasiakhir')");
		if($qry)
		{
			{
				?>
