...
if(!empty($nama)) //jika ada file yang di attach
{
	...
	$lokasiakhir="assets/img/tim/$namafile"; //untuk di database;
	$lokasifile="../../assets/img/tim/$namafile"; //untuk di folder;
	...
	if (!in_array($tipe, $allowedExts))
	{
		...
		<?php echo"window.location='../home.php?p=tim&addtim'"; ?>
		</script>
		<?php
		exit();
	}
	...
	{ //jika filenya benar
		// $qry = mysql_query("select idtim from tim");
		...
		$qry = mysql_query("insert into tim(name,position,url) values ('$judul','$cp','$lokasiakhir')");
		if($qry)
		{
			{
				?>
