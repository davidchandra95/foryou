...
if(!empty($nama)) //jika ada file yang di attach
{
	...
	$lokasiakhir="assets/img/author/$namafile"; //untuk di database;
	$lokasifile="../../assets/img/author/$namafile"; //untuk di folder;
	...
	if (!in_array($tipe, $allowedExts))
	{
		...
		<?php echo"window.location='../home.php?p=author&addauthor'"; ?>
		</script>
		<?php
		exit();
	}
	...
	{ //jika filenya benar
		// $qry = mysql_query("select idauthor from author");
		...
		$qry = mysql_query("insert into author(name,position,url) values ('$judul','$cp','$lokasiakhir')");
		if($qry)
		{
			{
				?>
